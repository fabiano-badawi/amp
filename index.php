...
if (isset($_GET['releaseid'])) {
	...
	$field = 'ReleaseId';
	$value = $_GET['releaseid'];

} elseif (isset($_GET['urlslug'])) {
...
$obj = getNewsDetails($field, $value);
...
<head>
    <meta charset="utf-8">
    <title><?=$obj->Title?> - Negócios em Foco</title>
<?php if($obj->Active == 0 || $obj->indexed == 0) { ?>
    <meta name="googlebot" content="noindex, nofollow">
<?php } ?>
    <meta name="viewport" content="width=device-width,minimum-scale=1,maximum-scale=1,initial-scale=1">
    <meta property="og:title" content="<?=str_replace('"', '', encurtador($obj->Body,70))?>" />
<?php if(!empty($obj->MainPictureUrl)) { ?>
    <meta property="og:image" content="https://negociosemfoco.com/assets/newsimg<?=($obj->Source == 'Dino' ? '' : '_ngfoco')?>/<?=$obj->MainPictureUrl?>" />
<?php } ?>
    <link rel="canonical" href="https://negociosemfoco.com/amp/?releaseid=<?=$obj->ReleaseId?>">
    ...
    <!--*
        ...
        **-->
<?php if(!empty($obj->VideoUrl) && strpos($obj->VideoUrl, 'youtube.com') !== FALSE) { ?>
...
<?php } ?>
	...
	"headline": "<?=$obj->Title?>",
	"datePublished": "<?=$obj->PublishedDate?>",
	"author": {
	  "@type": "Organization",
	  "name": "<?=($obj->Source == 'Dino' ? 'Dino' : 'Negócios em Foco')?>"
	},
	"publisher": {
	  "@type": "Organization",
	  "name": "Negócios em Foco",
	  "logo": {
	    "@type": "ImageObject",
	    "url": "https://negociosemfoco.com/amp/assets/img/logo-negocios-em-foco.png",
	    "width": 266,
	    "height": 60
	  }
	},
	"image": {
	  "@type": "ImageObject",
	  "url": "https://negociosemfoco.com/assets/newsimg<?=($obj->Source == 'Dino' ? '' : '_ngfoco')?>/<?=$obj->MainPictureUrl?>",
	  "height": <?=$obj->imgSize[1]?>,
	  "width": 696
      }
    }
    </script>
		...
		<div class="row">
			...
			<div class="col-sm-6">
				<div class="bordered-title">
					...
					<?php if(!empty($obj->Summary)) {
						if($detect->isMobile()) {
					?>
					<h5><?=encurtador($obj->Summary,130)?></h5>
					<?php } else { ?>
					<h4><?=$obj->Summary?></h4>
					<?php } }
						if (isset($_GET['home']) && ($_GET['home'] === '1' || $_GET['home'] === '0')) {
							setHomePage($_GET['releaseid']);
						}

						if($obj->Active == 0 || isset($_GET['confirm'])) {

						if(isset($_GET['confirm']) && $_GET['confirm'] / 13 == $_GET['releaseid'] && $obj->Confirmed == 0) {
							confirmaNoticia($_GET['releaseid']);
					?>
					<button class="button button-large grass-bg light-color"><i class="fa fa-check"></i> Você confirmou a sua notícia!</button><br />
					<?php } ?>

					<?php if(isset($_GET['confirm']) && $obj->Confirmed == 1) { ?>
					<button class="button button-large grass-bg light-color">Esta notícia já foi confirmada!</button><br />
					<?php } ?>

					<button class="button button-large grass-bg light-color">
						<i class="fa fa-hand-paper-o"></i> 
						Aguarde a aprovação do editor.<br />
						A sua notícia será publicada somente após aprovação do editor.<br />
						Você receberá o aviso de publicação em seu e-mail.
					</button>
					<?php } ?>
					...
					<h5><?=(empty($obj->Place) ? '' : '<i class="fa fa-map-marker"></i>')?> <?=$obj->Place?> <i class="fa fa-calendar"></i>
					    <?=date('d/m/Y', strtotime($obj->PublishedDate)) . ' <i class="fa fa-clock-o"></i> ' . date('H\hi', strtotime($obj->PublishedDate))?>
					</h5>
				</div><!-- TITLE ENDS -->
					...
					<div class="imagem-principal">
                                        	<amp-img src="/assets/newsimg<?=($obj->Source == 'Dino' ? '' : '_ngfoco')?>/<?=$obj->MainPictureUrl?>" width=<?=$obj->imgSize[0]?> height=<?=$obj->imgSize[1]?> layout="responsive"></amp-img>
					</div>
                                <?php } ?>
...
<?php if($obj->Source == 'Dino') { ?>
				<p>Divulgado por <amp-img src="/wp-content/uploads/2017/05/logo-dino-32.png" width="111" height="32"></amp-img></p>
<?php } ?>
				...
				<?php if($detect->isMobile()) { ?>
					<amp-ad width="100vw" height="320"
					     type="adsense"
					     data-ad-client="ca-pub-3025971466139023"
					     data-ad-slot="8827944450"
...
<?php
$lastNews = array(
'2692574;<h4 class="margin-0">Home Office</h4><span><strong>como adaptar a casa para ser produtivo</strong></span>',
'1193687;<h4 class="margin-0">Saúde mental</h4><span><strong>neurocirurgiã explica como manter em dia</strong></span>',
'1711301;<h4 class="margin-0">Marketing</h4><span><strong>mantenha a sua marca viva</strong></span>');
shuffle($lastNews);
?>

                                <div>
                                        <h3 class="margin-0">Publicações Recentes</h3>

                                        <ul class="media-list">
						<?php foreach(array_slice($lastNews, 0, 3) as $value) {
							$value = explode(';', $value);
						?>
                                                <li>
                                                        <a class="clearfix" href="/amp/?releaseid=<?=$value[0]?>">
                                                                <div>
                                                                        <?=$value[1]?>
                                                                </div>
